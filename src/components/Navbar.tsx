  ...
  return (
    <>
      <nav className="navbar" id="main-navbar">
        <div className="navbar-inner">
          ...
          <div className="navbar-actions">
            <Link to="/profile" className="navbar-action-item desktop-only" id="profile-btn" aria-label="Profile">
              <User size={20} />
              <span className="navbar-action-label">Profile</span>
            </Link>
            <Link to="/messages" className="navbar-action-item desktop-only" id="message-btn" aria-label="Messages">
              <MessageSquare size={20} />
              <span className="navbar-action-label">Message</span>
            </Link>
            <Link to="/orders" className="navbar-action-item desktop-only" id="orders-btn" aria-label="Orders">
              <Package size={20} />
      ...
      <aside className={`mobile-sidebar ${mobileOpen ? 'open' : ''}`}>
        <div className="sidebar-header grey-header">
          ...
          <button className="close-sidebar" onClick={() => setMobileOpen(false)}>
            <X size={24} />
          </button>
        </div>
        ...
        <div className="sidebar-content">
          <div className="sidebar-section">
            <Link to="/" onClick={() => setMobileOpen(false)}><Menu size={18} /> Home</Link>
            <Link to="/categories" onClick={() => setMobileOpen(false)}><Menu size={18} /> Categories</Link>
            <Link to="/favorites" onClick={() => setMobileOpen(false)}><Heart size={18} /> Favorites</Link>
            <Link to="/orders" onClick={() => setMobileOpen(false)}><Package size={18} /> My orders</Link>
          </div>

          <div className="sidebar-divider"></div>

          <div className="sidebar-section">
            <button className="sidebar-btn" onClick={() => {}}><Globe size={18} /> English | USD</button>
            <Link to="/contact" onClick={() => setMobileOpen(false)}><PhoneCall size={18} /> Contact us</Link>
            <Link to="/about" onClick={() => setMobileOpen(false)}><Info size={18} /> About</Link>
          </div>
          
          <div className="sidebar-divider"></div>

          <div className="sidebar-section text-only-links">
            <Link to="/agreement" onClick={() => setMobileOpen(false)}>User agreement</Link>
            <Link to="/partnership" onClick={() => setMobileOpen(false)}>Partnership</Link>
